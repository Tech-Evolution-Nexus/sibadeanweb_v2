<x-app-layout :title="$data->title">
    ...
    <div class="md:px-12 px-6 md:py-10 py-5">
        <div class="mb-10">
            <div class="text-sm">Dashboard / <a class="hover:underline" href="{{route("surat.index")}}">Kartu keluarga</a> / {{$data->title}}</div>
            <div class="flex">
        ...
        <form action="<?= $data->action_form ?>" method="POST" class="" enctype="multipart/form-data">
            ...
            <div class="grid md:grid-cols-2 grid-cols-1 gap-2 gap-x-4">
                ...
                <div class=" mb-2 ms-md-3">
                    ...
                    <x-text-input :value="old('nama_surat', $data->data->nama_surat)" type="text" class="block mt-1 w-full" placeholder="Nama Surat" name="nama_surat" id="nama_surat" required />
            ...
            <div class="flex md:justify-end md:flex-row flex-col-reverse mt-8 gap-4">
                <a href="{{route("surat.index")}}" class="px-4 md:w-auto w-full py-2 bg-slate-200 text-center rounded-md text-gray-900 ">Kembali</a>
