    ...
    <div class="md:px-12 px-6 md:py-10 py-5">
        ...
        <form action="<?= $data->action_form ?>" method="POST" class="" enctype="multipart/form-data">
            ...
            <div class="grid md:grid-cols-2 grid-cols-1 gap-2 gap-x-4">
                <div class="mb-2">
                    ...
                    @if($data->data->nama_file)
                        <p class="mb-2 text-sm">
                            <a href="{{ route('surat-keluar.download', $data->data->nama_file) }}"
                    ...
                    <input type="file" name="nama_file" id="nama_file" accept="application/pdf"
                ...
                <div class=" mb-2">
                    <x-input-label for="exp_date" :value="__('Tanggal Acara')" />
                    <x-text-input :value="old('exp_date', \Carbon\Carbon::parse($data->data->exp_date)->format('Y-m-d'))" type="date"
                        class="block mt-1 w-full" name="exp_date" id="exp_date" required />
