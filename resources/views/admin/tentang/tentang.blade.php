<x-app-layout>
    ...
    <div class="md:col-span-2">
        ...
        <select id="app_type" name="app_type" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" onchange="toggleAppInput(this)">
            <option value="upload" {{ $value->app_type == 'upload' ? 'selected' : '' }}>Upload</option>
            <option value="custom" {{ $value->app_type == 'custom' ? 'selected' : '' }}>Custom</option>
        </select>
    </div>

    {{-- Jika upload --}}
    <div class="md:col-span-2" id="upload_section" style="{{ $value->app_type == 'upload' ? '' : 'display:none;' }}">
        <label class="block text-sm font-medium text-gray-700">Upload Aplikasi Android/iOS</label>
       <x-file-upload 
        :name="'app_file'" 
        accept=".apk,.ipa"
    />
    ...
    {{-- Jika custom --}}
    <div class="md:col-span-2" id="custom_section" style="{{ $value->app_type == 'custom' ? '' : 'display:none;' }}">
        <label for="app_url" class="block text-sm font-medium text-gray-700">Custom URL:</label>
        <x-text-input type="text" value="{{ $value->mobile_link }}" class="mt-1 block w-full" id="app_url" name="app_url" />
    </div>
    </div>
